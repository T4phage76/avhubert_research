...
class TemporalCausalHead(nn.Module):
    ...
    def __init__(self, ch=128, num_layers=2, kernel_size=3, dropout=0.1, dilation_base=2):
        super().__init__()
        blocks = []
...
class CausalVisualEncoder(nn.Module):
    ...
    Output: (B, T, 128)   each t depends only on frames <= t
    """
...
class CrossAV(nn.Module):
    def __init__(
        ...
        self.visual_encoder = self.visual_encoder = CausalVisualEncoder(
            in_channels=1,
