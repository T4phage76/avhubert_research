...
# ----------- Make the model feature dim to be 256 -----------


# ---- Auditory ENcoder ---- (Consider making this into causal as well?)
class AuditoryEncoder(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.encoder = nn.Sequential(

            nn.Conv2d(in_channels, 64, kernel_size=(3, 3), stride=1, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
...
class TemporalBlock(nn.Module):
    """Causal residual block: Conv -> Norm -> ReLU -> Drop -> Conv -> Norm -> ReLU -> Drop + Skip"""
    def __init__(self, ch, kernel_size=3, dilation=1, dropout=0.1):
        super().__init__()
        self.conv1 = CausalConv1d(ch, ch, kernel_size, dilation=dilation)
        self.bn1   = nn.BatchNorm1d(ch)
        self.conv2 = CausalConv1d(ch, ch, kernel_size, dilation=dilation)
    ...
    def forward(self, x):  # (B, C, T)
        ...
        return y  # residual here? x + y? maybe not because it's shallow

class TemporalCausalHead(nn.Module):
    """Stack of dilated causal blocks to aggregate past context."""
    def __init__(self, ch=256, num_layers=2, kernel_size=3, dropout=0.1, dilation_base=2):
        super().__init__()
        blocks = []
        for i in range(num_layers):
            d = dilation_base ** i
            blocks.append(TemporalBlock(ch, kernel_size=kernel_size, dilation=d, dropout=dropout))
        self.net = nn.Sequential(*blocks)

    @property
    def receptive_field(self):
        # RF across the stack (frames looked back)
        # sum_i ( (k-1)*d_i ) + 1
        rf = 1
        for m in self.net:
            if isinstance(m, TemporalBlock):
                # two convs per block
                # each adds (k-1)*d to RF
                k1 = m.conv1.kernel_size[0]; d1 = m.conv1.dilation[0]
                k2 = m.conv2.kernel_size[0]; d2 = m.conv2.dilation[0]
                rf += (k1 - 1) * d1 + (k2 - 1) * d2
        return rf
...
class CausalVisualEncoder(nn.Module):
    """
    Causal visual encoder: per-frame 2D CNN + causal temporal head.
    Input:  (B, T, H, W)  grayscale, float
    Output: (B, T, 256)   each t depends only on frames <= t
    """
    def __init__(self, in_channels=1, use_dropout=True, p=0.1,
                 tcn_layers=2, tcn_kernel=3, tcn_dropout=0.1, tcn_dilation_base=2):
        super().__init__()

        # Spatial per-frame backbone (same as the old encoder)
        ...
        # Causal temporal head over time (keeps channel=128)
        self.temporal = TemporalCausalHead(
            ch=256,
...
class CrossAV(nn.Module):
    def __init__(
        ...
        self.visual_encoder = CausalVisualEncoder(
            in_channels=1,
            use_dropout=True, p=0.1,
            tcn_layers=1,        # 1 layer = 1 TCN blcok = 2 conv1D layers
            tcn_kernel=3,       
            tcn_dropout=0.1,
            tcn_dilation_base=2   
        )
...
# class AuditoryEncoder(nn.Module):
#     def __init__(self, in_channels, dim_model_feat):
#         self.dim_model_feat=128
#         super().__init__()
#         self.encoder = nn.Sequential(
...
#             nn.Conv2d(256, self.dim_model_feat, kernel_size=(3, 3), stride=1, padding=1),
#             nn.BatchNorm2d(self.dim_model_feat),
#             nn.ReLU(),
